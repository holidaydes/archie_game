...
public class ArchieBackgroundAnimation {
    ...
    public void render(SpriteBatch batch) {
        float x = ((gameCamera.viewportWidth / 2 - gameCamera.position.x) * scale) % background.getRegionWidth();
        float y = ((gameCamera.viewportHeight / 2 - gameCamera.position.y) * scale) % background.getRegionHeight();
